...
const genUserObj = async () => {
    const users = await User.findAll({attributes: ['id']})

    return users.reduce((accum,user)=> {
        accum[user.id] = {id: user.id, count:0}
        return accum
    },{})
}
...
const getReviewText = (reviews) =>{
    let reviewText = reviews[Math.floor(Math.random()*reviews.length)]

    reviews.splice(reviews.indexOf(reviewText))
...
const genReviews = async () =>{
    ...
    let gameStuff = []
    let gameTitles = await Game.findAll({attributes: ['id','title']})

    gameTitles.forEach(game => gameStuff.push({title: game.title, id: game.id}))

    let users = await genUserObj()
    let results = []

    let reviews = []

    for (let game of gameStuff){

        let searchResult = await getWiki(game.title,game.id)
        results.push(searchResult)
    }

    for (let result of results){
        let gameId = Object.keys(result)[0]
